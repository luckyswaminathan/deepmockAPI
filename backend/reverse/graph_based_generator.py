...
class GraphDataGenerator:
    ...
    def _build_record(
        ...
        for prop in properties:
            ...
            # Skip "id" field - it will be set with component name prefix at the end
            if prop_name == "id":
                continue
        ...
        # Always ensure ID has correct format (component name prefix)
        # This overrides any id generated by _generate_field_value
    ...
    def _generate_with_gpt(
        ...
        # Don't use GPT for "id" field - it will be set with component name prefix
...
Value:"""
    ...
    def _generate_heuristic(
        ...
        # ID fields (skip plain "id" - handled separately in _build_record)
        if field_lower == "id":
            # This shouldn't be called for "id" field, but if it is, return None
            return None
        if field_lower.endswith("_id"):
            # For fields like customer_id, use the field name as prefix
            prefix = field_lower.replace("_id", "")
            return f"{prefix}_{index}"
        if field_lower == "uuid":
            return f"uuid_{index}"
